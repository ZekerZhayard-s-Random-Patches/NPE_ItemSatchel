...
public class ItemSatchel extends ItemMulti implements IInitializer, IColorableItem, IEnchantableItem, IInventoryContainerItem, IMultiModeItem {
    ...
    public void registerModels() {
        System.out.println("ZekerZhayard Modified");
        ModelLoader.setCustomMeshDefinition(this, stack -> {
            return new ModelResourceLocation(
                getRegistryName(),
                String.format(
                    "access=%s,color0=%s,color1=%s,type=%s",
                    SecurityHelper
                        .getAccess(stack)
                        .toString()
                        .toLowerCase(Locale.US),
                    ColorHelper.hasColor0(stack) ? 1 : 0,
                    ColorHelper.hasColor1(stack) ? 1 : 0,
                    typeMap.get(
                        ItemHelper
                            .getItemDamage(stack)
                    ).name));
